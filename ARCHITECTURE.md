...
文档整理：2026-09-10。当前是静态档案＋本地词项检索＋项目状态；不是模型微调，未接入向量模型。
...
历史轮次的“没有 push”仅描述当时状态；本次从远程 main（8fcb39a）重新克隆，已包含此前功能。
具体发布状态以远程提交为准。用户要求发布时可用正常 git；网络失败再用官方 API，
...
README 改为面向首次使用者的说明，START_HERE 为无上下文模型提供加载顺序、能力判断与无工具降级。
docs/GETTING_STARTED、FAQ、FILE_MAP 说明安装、公开/私有资产边界、依赖、迁移和按需阅读；
CONTRIBUTING 约束文档联动与验证。SKILL 保留正式执行纪律，不把维护日志放入每次模型上下文。
静态使用没有 Python/Zotero 前置要求；PDF/OCR 和开发行为评测的可选依赖必须明确。
本节以上带“本轮”的数量与状态是历史快照，不能作为新用户数据资产或当前远程状态的证明。

### Skill 分发格式修订

README 以克隆→加载 SKILL→自然语言使用为主线，不展示 Python 入门步骤。
SKILL 是唯一执行入口；START_HERE 仅兼容旧链接。安装目录命名 misq-reviewer，与 frontmatter 一致。
保留单 skill 根目录与 tools 路径，避免为外观重排破坏脚本和历史命令；Agent Skills 规范允许额外目录。
研究工作流、期刊档案、用户文档、维护记录各自分层。格式对照与验证见 examples/DISTRIBUTION.md。
